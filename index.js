...
const readthousands = (number) => {
  switch (number.length) {
    case 4:
      ...
      return `${main_nums[number[0]]} آلالف و ${readGroup(
    ...
    case 6:
      if (number !== "000000") {
        ...
        )} ألفاً و ${readGroup(number.slice(number.length - 3))}`;
      }
      return "";
  }
};
const readmillions = (number) => {
  switch (number.length) {
    case 7:
      ...
      return `${main_nums[number[0]]} ملايين و ${readthousands(
        number.slice(1)
      )}`;
    case 8:
    case 9:
      if (number !== "000000000") {
        return `${readGroup(number.slice(0, number.length - 6))}  ${
          units[2]
        }اً و ${readthousands(number.slice(number.length - 6))}`;
      }
      return "";
  }
};
const readbillions = (number) => {
  switch (number.length) {
    case 10:
      ...
      return `${main_nums[number[0]]} ${units[3]} و  ${readmillions(
        number.slice(1)
      )}`;
    case 11:
    case 12:
      if (number !== "") {
        return `${readGroup(number.slice(0, number.length - 9))}  ${
          units[3]
        } اً و ${readmillions(number.slice(number.length - 9))}`;
      }
      return "";
  }
};
